...
func TestListInstanceProfilesPagination(t *testing.T) {
	...
	profileNames := []string{*profiles.Profiles[0].Name, *profiles.Profiles[1].Name}
	if !reflect.DeepEqual(profileNames, []string{"bx2-2x8", "gx2-8x64x1v100"}) {
		t.Fatalf("unexpected profiles: %v", profileNames)
	}
	if !reflect.DeepEqual(starts, []string{"", "page-two"}) {
		t.Fatalf("expected page starts [\"\" \"page-two\"], got %v", starts)
	}
